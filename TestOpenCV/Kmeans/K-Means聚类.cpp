...
void CaptureFromCG(IplImage *img){

	CGSTATUS status = CG_OK;
	status = BeginCGCard(1,&pHandle);						//开始指定图像卡操作，初始化图像卡，获得其设备句柄，分配相应的资源
	CGSetVideoFormat(pHandle, RGB888);
	CGSetInputWindow(pHandle, 0, 0,768,576);					
	CGSetOutputWindow(pHandle,0, 0, OutputWidth, OutputHeight);
	DWORD dwImageSize = 0;									//图像大小
	BITMAPINFO *pBmpInfo	= NULL;							//BITMAPINFO 结构指针，显示图像时使用
	BYTE *pInfoBuffer		= NULL;		
	BYTE *pImageBuffer		= NULL;							//BIMTAPINFO 存储缓冲区，m_pBmpInfo即指向此缓冲区
	BYTE *pStaticBuffer		= NULL;							//静态内存地址指针
	HANDLE handle			= NULL;							//静态内存描述句柄
	int nNumber = -1, nStatus = 0;
	pInfoBuffer = new BYTE[sizeof(BITMAPINFO)];
	pImageBuffer = new BYTE[ OutputWidth * OutputHeight * 3];
	img->origin = 1;
	pBmpInfo = (BITMAPINFO *)pInfoBuffer;					
	pBmpInfo->bmiHeader.biSize			= sizeof(BITMAPINFOHEADER);
	pBmpInfo->bmiHeader.biWidth			= OutputWidth;				//	图像宽度，一般为输出窗口宽度
	pBmpInfo->bmiHeader.biHeight		= OutputHeight;				
	...
	dwImageSize = OutputWidth * OutputHeight * 3;							//	计算图像大小，由视频输出窗口和视频格式确定
	status = CGStartSnap(pHandle, 0, TRUE, 2);	
	while(nNumber==-1){
		CGGetSnappingNumber(pHandle, &nStatus);				
		if (nStatus == 2)  									//第1帧采集完成，正在采集第2帧
			nNumber		= 0;
		if (nStatus == 0)  									//第2帧采集完成，正在采集第1帧
			nNumber		= 1;
		if (nNumber > -1)
		{
			status = CGStaticMemLock(dwImageSize * nNumber, dwImageSize, &handle, (VOID **)&pStaticBuffer);
			CGDataTransform(pImageBuffer, pStaticBuffer, OutputWidth, OutputHeight, 24, TRUE);
			CGStaticMemUnlock(handle);						//解除静态内存锁定
			cvSetData(img,pImageBuffer, OutputWidth * 3);
		}
		int c = cvWaitKey(33);
	}
}
